...
| Double-O-Seven |
| |
| Esta secuencia de comandos de Anti-Cheat está hecho por mí, Double-O-Seven. Los famosos |
| Sistema anti-Cheat "PunkBuster" me inspiró el guión de una nueva |
| Guión anti-trucos para SA: MP. Contiene muchas funciones contra |
| Trucos normales y los tramposos de herramientas de trucos específicos. Va en contra |
| (Comando) correo no deseado, también. |
| Si JunkBuster patea / prohibiciones demasiada jugadores inocentes, deshabilitar los códigos |
| Que patear / prohibir demasiados jugadores inocentes. Puede fácilmente configurate |
| JunkBuster en el archivo "JunkBuster.cfg" en la carpeta "JunkBuster" |
| En la carpeta "scriptfiles". Use "/ jbcfg" dentro del juego si usted es |
| (Rcon) administrador para actualizar la configuración. |
| "ForbiddenWeapons.cfg" a las armas prohibidas. |
| |
| El script principal de JunkBuster ya no es un include! |
| Es un filterscript ahora! Utilice JunkBuster.inc para todas sus |
| Otros scripts o algo no funcione como debería. |
| |
| Este guión ha sido realizado por Double-O-Seven! |
| |
| Usted no está autorizado a: |
| - Retire este texto |
| - Rename JunkBuster! ¡Nunca! |
| - Re-lanzar este script |
| - Digamos que es su propio guión |
| - Quitar el "JunkBuster:" tag de mensajes del cliente! |
| |
| Sin embargo, se le permite añadir más funciones y utilizarlo en su servidor. |
| |
| Gracias a Cueball ~ ~ por su guión zona, ZeeX para ZCMD |
| Y_Less y para la función sscanf. También doy las gracias por permitir dUDALUS |
| Me engaño en su servidor. |
| |
| (Si escribo SS en cualquier lugar, que significa "servidor", y no "Schutzstaffel". |
| Si escribo CS en cualquier lugar, que significa "del lado del cliente", no "Counter Strike"). |
+ ------------------------------------------------- ---------------------- +
* /
...
Cambios Update 11:
- Funciones Ban Temperatura Fija
- Código de sincronización reescrito
- SplitIp functino fija
- Cuando anti-Moneyhack está habilitada, JB desactivará automáticamente bonificaciones dobles
- Código anti-joypad reescrito (en realidad funciona ahora)
- Cuando un jugador entra en un vehículo, los componentes del vehículo que no deben estar en el vehículo conseguirán eliminado
- Arreglado un bug anti-tuning-hack: Los jugadores pueden recibir una patada cuando el vehículo fue destruido
- Se ha añadido más rápida detección de freno de aire. Usted puede elegir entre el viejo detección y el nuevo. El nuevo está activada por defecto. Es necesario reiniciar el servidor para cambiar el modo de detección.
- Ahora es totalmente compatible con LuxAdmin (utilizar # include <JunkBuster_Slim> en LuxAdmin)

Cambios Update 10:
- Double-O-bits en lugar de y_bit
- Anti-joypad
- Detección speedhack avanzado con límite de velocidad individual para cada vehículo
- Lucha contra vehículos armados
- Mejora de la anti-moneyhack
- Comando / jbreports para todos los administradores: listas de comandos los nuevos informes JB
- El uso de memoria optimizado
- SetJBVar, funciones GetJBVar
- Advertencias para la unidad de byers
- Sintaxis prefijo unificado (LLAMADA:, PRIVADO:, PUBLIC :)
- El código optimizado para IP-prohibiciones, listas negras, listas blancas y tempbans
- Si un administrador accidently tipos la contraseña rcon en el chat global, bloques junkbuster que
- Funciones de lista negra etc ya están disponibles para otros scripts
- Diálogo para hackear los códigos (Si no entiendes será después de leer el diálogo)
- El diálogo de lista de variables de color (verde = on, rojo = off)
- AdvertisementCheck recodificado
- Detección de vehículo de reparación truco
- Detección de afinación truco
- Detectar si un jugadores tramposos o consigue un arma de forma gratuita en Ammu-Nation
- Hecho de enganche fácil en JunkBuster.inc. No es importante para ti, sólo para mí.
- Sincronización de tiendas de salud / blindaje mejorado. Salud ahora queda sólo sincroniza en los restaurantes y armaduras sólo en Ammu-Nation en lugar de los dos lugares.
- Las armas también sólo se sincronizan en Ammu-Nation en lugar de todas las tiendas. Además, sólo las armas comprables obtener clientside sincronizado.
- GodModeCheck reescrito. Ahora debería funcionar mejor.
- Anti-AFK? ¿O era que ya en la actualización 9 ..?
- Aerofrenos Ahora bien escrita. También me ha permitido dar la AIRBRAKE variable de una nueva función como límite inferior de velocidad para freno de aire sin causar problemas en que ya existen servidores que tendrían límite de velocidad de 1 ... w / e
- Nueva detección de desove del vehículo (el método antiguo no hizo absolutamente nada en SA-MP 0.3)
- Una de las funciones GlobalUpdate eliminado.
- JunkBuster puede ahora tomar medidas cuando un jugador tiene muchas advertencias en total
...
/ *
Un nuevo sistema de prohibición de usar SQLite. Es mucho más lento que el sistema de prohibición de edad y no se puede utilizar el sistema de rangeban de JB, que
tiene la ventaja de contar con una lista blanca la hacen excepciones para algunos jugadores.
Elimine la siguiente línea a utilizar una base de datos para la prohibición de los jugadores.
(Tempbans, prohibiciones IP estáticas (sólo con un nombre asociado) y lista negra compatible. Lista blanca está un poco también apoyó ...)
Por el momento no veo ninguna ventaja en el uso de la base de datos a excepción de la cantidad de personas unlimitted puede prohibir con él.
El sistema de archivos está limitted por MAX_JB_BANS. Base de datos no lo es. Ah, antes de que me olvide, la base de datos no también no funciona correctamente
con algunos nombres con símbolos especiales.
>> Es sólo experimental. (Sin embargo, se puede hacer que funcione correctamente si usted tiene que saber.)
*/
...
PRIVATE: JB::MutePlayer (playerid, time, reason [])//Time is seconds.
{
	if (IsPlayerConnected (playerid) && time)
	{
		...
		JB::LogEx ("%s has been muted for %d second(s) because of %s.", JB::PlayerInfo [playerid][JB::pName], time, reason);
		JB::SendFormattedMessageToAll (JB_RED, "JunkBuster:{FFFF00} Player silenciado '%s' (%d) por %d segundos(s). Razón: %s", JB::PlayerInfo [playerid][JB::pName], playerid, time, reason);
		return 1;
	}
	return 0;
}
...
PRIVATE: JB::Kick (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		JB::SendFormattedMessageToAll (JB_RED, "JunkBuster:{FFFF00} El player '%s' ha sido kickeado. Razón: %s", JB::PlayerInfo [playerid][JB::pName], reason);
		JB::LogEx ("%s (%s) has sido kickeado por %s.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp], reason);
...
PRIVATE: JB::Ban (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		TogglePlayerControllable (playerid, false);
		SetCameraBehindPlayer (playerid);
		JB::SendFormattedMessageToAll (JB_RED, "JunkBuster:{FFFF00} El player '%s' ha sido baneado. Razón: %s", JB::PlayerInfo [playerid][JB::pName], reason);
		JB::LogEx ("%s (%s) has sido baneado por %s.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp], reason);
...
PRIVATE: DisplayKickBanText (playerid, reason [], bool: ban)
{
	if (IsPlayerConnected (playerid))
	{
		...
		format (string, sizeof (string), "~y~Nombre:~n~~w~%s~n~~y~Fecha:~n~~w~%s~n~~y~Hora:~n~~w~%s~n~~y~IP:~n~~w~%s~n~~y~Razon:~n~~w~%s", JB::PlayerInfo [playerid][JB::pName], JB::GetDate (), JB::GetTime (),
		...
		format (string, sizeof (string), "~g~Si usted ha sido injustamente %s, por favor presiona ~b~F8~g~ para hacer una captura de pantalla y quejarse en el ~b~foro~g~ de este server.", ban ? ("baneado") : ("kickeado"), JB::Homepage);
...
PUBLIC: CheckText (playerid, text []) //return 1: something forbidden found, return 0: everything OK!
{
	...
	{
		SendClientMessage (playerid, JB_RED, "JunkBuster:{FFFF00} No está permitido el uso de bloqueo de mayúsculas! Pulse [Caps Lock] para desactivarlo.");
		return 1;
	}
	...
	{
		SendClientMessage (playerid, JB_RED, "JunkBuster:{FFFF00} No está permitido el uso de palabras tales!");
		return 1;
	}
	return 0;
}
...
PRIVATE: JunkBusterReport (playerid, report [], details [])
{
	if (IsPlayerConnected (playerid))
	{
		...
		format (string, sizeof (string), "JunkBuster:{FFFF00} Sospecha de player con %s: %s (%d, %s), %s.", report, JB::PlayerInfo [playerid][JB::pName], playerid, JB::PlayerInfo [playerid][JB::pIp], details);
		ForEachPlayer (i)
    ...
    ForEachPlayer (i)
    {
        ...
        if (DOB::GetBit (JB::FullyConnected, i))
        {
				...
				&& (nokeypressed || IsPlayerInRangeOfPoint (i, 3.0, JB::PlayerInfo [i][JB::pAFKPos][0], JB::PlayerInfo [i][JB::pAFKPos][1], JB::PlayerInfo [i][JB::pAFKPos][2])))
	        {
	            ...
	            {
	                format (reason, sizeof (reason), "Inactivo durante demasiado tiempo [Max %d min(s)]", JB::Variables [AFK]);
	                JB::Kick (i, reason);
	            }
	        }
			else
			...
			if (!JB::IsPlayerAdmin (i) && IsPlayerInValidState (i) && !DOB::GetBit (JB::AntiBugKilled, i)) //!JB::PlayerInfo [i][JB::pAntiBugKilled])
			{
			    ...
			    if (JB::Variables [JOYPAD] && !JB::PlayerInfo [i][JB::pFreezeTime])
				{
				    ...
				    if((updown != 128 && updown != 0 && updown != -128) || (leftright != 128 && leftright != 0 && leftright != -128))
				    {
						...
						else
						{
						    ...
						    SetSyncTime (i, SYNC_TYPE_VEHICLE);
							TogglePlayerControllable (i, false);
							SendClientMessage (i, JB_RED, "JunkBuster:{FFFF00} Por favor, dejar de utilizar un joypad / gamepad. Utiliza el teclado y el ratón.");
						}
				    }
				}
				...
				if (JB::Variables [MIN_FPS])
				{
					...
					if (var < JB::Variables [MIN_FPS])
					{
						...
						{
							JB::SendFormattedMessage (i, JB_RED, "JunkBuster:{FFFF00} Por favor, corrija tus FPS (FPS) o te kickeo! (Min %d, tus FPS: %d)", JB::Variables [MIN_FPS], var);
							SendClientMessage (i, JB_RED, "JunkBuster:{FFFF00}Al pulsar F7 vez puede ayudar a solucionarlo. (Elimina el contorno de las letras en la ventana de chat.");
						}
					}
				}
				...
				if (JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_DRIVER)
				{
					...
					if (JB::Variables [MAX_SPEED])
					{
						...
						{
						    if (JB::Variables [SPEEDHACK_DETECTION] == 1)
						    {
								...
								if (!JB::PlayerInfo [i][JB::pFreezeTime])
								{
									if (var > JB::Variables [MAX_SPEED])
									{
										...
										JB::PlayerInfo [i][JB::pFreezeTime] = 10; // Freeze for 5 seconds.
										SetSyncTime (i, SYNC_TYPE_VEHICLE);
										TogglePlayerControllable (i, false);
										if (JB::Warnings [i]{MAX_SPEED} < MAX_CHECKS)
										{
											format (reason, sizeof (reason), "%d KM/H con %s (Max %d)", var, JB::GetVehicleName (vehicleid), ivar [0]);
				...
				if (JB::Variables [WEAPON_HACK] && !JB::SyncInfo [i][SYNC_TYPE_WEAPONS][JB::sSyncTime])
				{
					if (var >= 16 && var <= 39)//Grenades and guns
					{
						...
						{
							GetWeaponName (var, reason, sizeof (reason));
							format (reason, sizeof (reason), "Usando trucos de armas (%s)", reason);
							JB::Ban (i, reason);
							continue;
						}
					}
				...
				if (JB::PlayerInfo [i][JB::pPingCheckProgress] == MAX_PING_CHECKS)
				{
					if (JB::Variables [MAX_PING])
					{
						...
						{
							format (reason, sizeof (reason), "Demasiado ping alto. [%d/%d]", var, JB::Variables [MAX_PING]);
							JB::Kick (i, reason);
							continue;
						}
					}
		...
		for (new i = 0; i < TempBanCount; ++i)
		{
			if (!strcmp (TempBanInfo [i][tbName], JB::PlayerInfo [playerid][JB::pName], false))
			{
				if (TempBanInfo [i][tbTime] > time)
				{
					...
					JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} You are temporary for %d day(s), %d hour(s), %d minute(s) and %d second(s)!", days, hours, minutes, seconds);
...
PRIVATE: ConfigJunkBuster ()
{
	...
	{
	    JB::Homepage = "the homepage of this server";
...
CALLBACK: OnPlayerReport (playerid, reporterid, report [])
{
	...
	JB::LogEx ("%s (%s) has reported %s (%s). Reason: %s", JB::PlayerInfo [reporterid][JB::pName], JB::PlayerInfo [reporterid][JB::pIp], JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp], report);
	...
	if (!JB::IsPlayerAdmin (playerid))
	{
		...
		if (JB::Variables [SPEEDHACK_DETECTION] != 0 && JB::PlayerInfo [playerid][JB::pCurrentState] == PLAYER_STATE_DRIVER && !JB::PlayerInfo [playerid][JB::pFreezeTime])
		{
			...
			if (speed > JB::Variables [MAX_SPEED])
			{
				...
				JB::PlayerInfo [playerid][JB::pFreezeTime] = 10; // Freeze for 5 seconds.
				SetSyncTime (playerid, SYNC_TYPE_VEHICLE);
				TogglePlayerControllable (playerid, false);
		    ...
		    if (JB::PlayerInfo [killerid][JB::pCurrentState] == PLAYER_STATE_DRIVER)
		    {
				if (JB::Variables [DRIVE_BY] >= 2 && (reason == WEAPON_UZI || reason == WEAPON_MP5 || reason == WEAPON_TEC9 || reason == 50)) // 50 = helicopter blades.
				{
					...
					else
					{
						...
						SendClientMessage (killerid, JB_RED, "JunkBuster:{FFFF00} Se le ha matado por drive-by!");
					}
				}
			 	...
			 	if (JB::Variables [ARMED_VEHICLES])
			 	{
					...
					if (IsArmedVehicle (vehicleid))
					{
					    if (++JB::Warnings [killerid]{ARMED_VEHICLES} >= JB::Variables [ARMED_VEHICLES])
						{
							...
							JB::SendFormattedMessage (killerid, JB_RED, "JunkBuster:{FFFF00} Matar con %s no esta permitido! !Basta! (Warning %d/%d)", JB::GetVehicleName (vehicleid), JB::Warnings [killerid]{ARMED_VEHICLES}, JB::Variables [ARMED_VEHICLES]);
					}
			 	}
			}
			...
			if (JB::Variables [SPAWNKILL])
			{
				if (JB::PlayerInfo [playerid][JB::pSpawnKillProtected])
				{
					if (++JB::Warnings [killerid]{SPAWNKILL} >= JB::Variables [SPAWNKILL])
						...
						JB::SendFormattedMessage (killerid, JB_RED, "JunkBuster:{FFFF00} Do not spawnkill! (Warning %d/%d)", JB::Warnings [killerid]{SPAWNKILL}, JB::Variables [SPAWNKILL]);
				}
		 	}
		}
...
CALLBACK: OnFilterScriptInit ()
{
	...
	JB::Log ("JunkBuster (v"#JB_VERSION") has successfully been loaded.");
	return 0;
}
...
CALLBACK: OnPlayerConnect (playerid)
{
	...
	if (!IsPlayerNPC (playerid))
	{
	        ...
	        if (JB::Variables [BLACKLIST])
	        {
		        ...
		        if (banned)
		        {
		            ...
		            {
		                SendClientMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Usted ha sido baneado de este server!");
						JB::LogEx ("%s (%s) has been banned for Ban evading.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp]);
					...
					else if (t < unbantime)
					{
					    ...
					    JB::SendFormattedMessageToAll (JB_RED, "JunkBuster:{FFFF00} You are temporary for %d day(s), %d hour(s), %d minute(s) and %d second(s)!", days, hours, minutes, seconds);
		        ...
		        {
		            SendClientMessage (playerid, JB_RED, "JunkBuster:{FFFF00} You are banned from this server!");
					JB::LogEx ("%s (%s) has been banned for Ban evading.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp]);
					JBDB::UpdateUserData (JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp], true, unbantime, whitelisted);
		            DOB::SetBit (JB::KickBan, playerid, true);
	                Kick (playerid);
	...
	{
		SendClientMessage (playerid, JB_GREEN_BLUE, "> Este servidor se está ejecutando con {FF0000}JunkBuster Anti-Cheat "#JB_VERSION"{00D799}!");
		SendClientMessage (playerid, JB_GREEN_BLUE, "> Usted no puede engañar de lo contrario te kickean/banean.");
	}
	return 0;
}
...
CALLBACK: OnPlayerText (playerid, text [])
{
	...
	{
	    JB::LogEx ("Player '%s' probablemente ha escrito accidentalmente el mando RCON en el chat. Mensaje bloqueado. (\"%s\")", JB::PlayerInfo [playerid][JB::pName], text);
	    return 0;
	}

	if (JB::PlayerInfo [playerid][JB::pMuted] && JB::Variables [SPAM])
	{
		JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} No tienes permitido chatear por %d segundo(s) porque has sido silenciado!", JB::PlayerInfo [playerid][JB::pMuted]);
		return 0;
	}
...
COMMAND:jbchrome (playerid, params [])
{
	...
	if (JunkBusterChrome (id, JB::PlayerInfo [playerid][JB::pName]))
		JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} JunkBuster Chrome has successfully collected data of player '%s'!", JB::PlayerInfo  [id][JB::pName]);
	else
	    JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} JunkBuster Chrome could not colect data of player '%s'!", params);
	return 1;
}
...
COMMAND:jbsethomepage (playerid, params [])
{
    ...
    DOF2::SetString (CONFIG_FILE, "Homepage", JB::Homepage, "Misc");
    JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} Homepage has been set to '%s'!", params);
	return 1;
}
...
COMMAND:blackadd (playerid, params [])
{
	...
	if (!IsPlayerConnected (id))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} Player '%s' has successfully been added to blacklist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Could not add player '%s' to blacklist!", params);
	}
	else
	{
		if (id != playerid)
		{
		        ...
		        {
		            JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} Player '%s' has successfully been added to blacklist!", JB::PlayerInfo [id][JB::pName]);
				...
				if (AddPlayerToBlacklist (id))
					...
					JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Could not add player '%s' to blacklist!", JB::PlayerInfo [id][JB::pName]);
		}
	}
	return 1;
}
...
COMMAND:blackdel (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} Player '%s' has successfully been removed from blacklist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Could not remove player '%s' from blacklist!", params);
			return 1;
	}
...
COMMAND:whiteadd (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} Player '%s' has successfully been added to whitelist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Could not add player '%s' to whitelist!", params);
			return 1;
	}
...
COMMAND:whitedel (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} Player '%s' has successfully been removed from whitelist!", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Could not remove player '%s' from whitelist!", params);
			return 1;
	}
...
COMMAND:jbsetvar (playerid, params [])
{
	...
	{
	    JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} JunkBuster variable '%s' has successfully been set to %d.", var, value);
...
COMMAND:tunban (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} Temporary ban of player '%s' has successfully been deleted.", params);
			else
				JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Could not delete temporary ban of player '%s'!", params);
			return 1;
	}
...
COMMAND:banip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
	    ...
	    SendClientMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Sorry, this command is currently not available when using JunkBuster database.");
	    return 1;
	#else
		if (!isnull (params))
		{
			...
			{
				if (BanIp (params))
					JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} IP %s has successfully been banned!", params);
				else
					JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Could not ban IP %s!", params);
				return 1;
			}
		}
...
COMMAND:unbanip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
	    ...
	    if (JBDB::UnbanIp (params))
			JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} IP %s has successfully been unbanned!", params);
		else
			JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Could not unban IP %s!", params);
		return 1;
	#else
		if (!isnull (params))
		{
				...
				case 24, 25, 27, 29, 30, 31, 33, 34: // Deagle, Shotung, SPAS12, MP5, AK47, M4, Rifle, Sniper Rifle
				{
					...
					else if (((oldkeys & KEY_FIRE) || (oldkeys == KEY_FIRE)) && ((newkeys & KEY_CROUCH) || (newkeys == KEY_CROUCH)) && (GetTickCount () - JB::PlayerInfo [playerid][JB::pFired]) < 750)
					{
						if (++JB::Warnings [playerid]{CBUG} & 1 == 0)
							SendClientMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Please stop performing the C-Bug or you will get kicked/banned.");
	...
	if (1 <= JB::Variables [DRIVE_BY] <= 2 && PRESSED (KEY_FIRE) && (HOLDING (KEY_LOOK_RIGHT) || HOLDING (KEY_LOOK_LEFT)))
	{
		if (!IsPlayerInPlane (playerid) && GetVehicleModel (GetPlayerVehicleID (playerid)) != 432)//Ignore planes and rhinos.
		{
			...
			if (weaponid != 0 && ammo != 0)//Check if player has got an SMG. Don't punish innoncent players.
			{
				...
				SendClientMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Please stop performing drive-by!");
				TogglePlayerControllable (playerid, false);
			...
			case DIALOG_SETVAR .. (DIALOG_SETVAR + MAX_JB_VARIABLES - 1):
			{
				if (response)
				{
					...
					else
					{
						...
						JB::SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} JunkBuster variable '%s' has successfully been set to %d.", JB::VariableNames [var], JB::Variables [var]);
